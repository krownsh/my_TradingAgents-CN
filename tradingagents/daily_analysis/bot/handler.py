...
from tradingagents.daily_analysis.bot.models import WebhookResponse
from tradingagents.daily_analysis.bot.dispatcher import get_dispatcher
from tradingagents.daily_analysis.bot.platforms import ALL_PLATFORMS

if TYPE_CHECKING:
    from tradingagents.daily_analysis.bot.platforms.base import BotPlatform
...
def handle_webhook(
    ...
    這是所有平台 Webhook 的統一入口。
    
    Args:
        platform_name: 平台名稱 (feishu, dingtalk, wecom, telegram)
        headers: HTTP 請求頭
        body: 請求體原始位元組
        query_params: URL 查詢參數（用於某些平台的驗證）
    ...
    logger.debug(f"[BotHandler] 請求數據: {json.dumps(data, ensure_ascii=False)[:500]}")
    ...
    # 如果沒有消息需要處理，返回空響應
    if not message:
        logger.debug("[BotHandler] 無需處理的消息")
        return WebhookResponse.success()
