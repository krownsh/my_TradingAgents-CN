...
class TPExProvider(MarketDataProvider):
    """
    台湾证券柜台买卖中心 (TPEx) 数据提供者 (MVP版)
    目前底层使用 yfinance 作为数据源，后续替换为 TPEx OpenAPI。
    """
    ...
    async def get_bars(
        ...
        # TPEx (OTC) stocks on Yahoo usually use .TWO suffix
        ticker = f"{symbol.code}.TWO"
    ...
    async def get_symbol_list(self) -> List[SymbolKey]:
        """
        获取 TPEx 股票列表 (从 isin.twse.com.tw)
        """
        try:
            ...
            logger.error(f"Error fetching TPEx list: {e}")
            return []
    ...
    async def get_sentiment(self, symbol: SymbolKey, **kwargs) -> str:
        ...
        sentiment_summary = f"""## 台灣櫃買市場情緒分析

**股票**: {symbol.code} (證券櫃檯買賣中心)
**分析日期**: {curr_date}
